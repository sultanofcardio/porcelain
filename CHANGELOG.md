...
- **The diff viewer follows `files.autoSave`** - the editable working-tree side now saves the way the setting says, in all four of its modes: `afterDelay` writes once the buffer has been quiet for `files.autoSaveDelay`, `onFocusChange` writes when the editor loses focus, `onWindowChange` when the window does, and `off` keeps Cmd+S as the only way to disk. Flipping the setting takes effect in an open diff. Autosave never writes while an IME composition is in flight, and it pauses while the changed-on-disk banner is asking which copy wins; a keystroke typed while a write is in flight stays dirty and goes out with the next save. The dirty dot keeps its meaning and its tooltip names the mode, and a save that fails says so in a banner instead of retrying on its own (#3)
- **Hover and go to definition in the diff viewer** - resting the pointer on a symbol shows the language server's hover, rendered from its markdown the way the editor's own hover widget renders it, with fenced code coloured. Holding Cmd (Ctrl elsewhere) underlines a symbol that has a definition, and a click on it, or F12 from the caret, opens the definition in the native editor, with a pick when there are several; Cmd+K Cmd+I shows the hover at the caret. `editor.hover.enabled` and `editor.hover.delay` apply. The working tree is asked about as the file on disk, history and index sides through their `porcelain:` documents, where the built-in JSON, CSS and HTML servers answer and TypeScript's does not. A side with unsaved edits stays quiet until the next save, and says so once when autosave is off (#3)
- **Fold rows name their scope** - a collapsed run's wavy row now carries the function or class its hidden lines start in, read from the document's symbol outline, ahead of the count, the way IntelliJ labels a fold. The merge editor's rows keep the count alone (#3)
...
- **Unchanged runs open in stages** - the fold row in the diff viewer and the merge editor is now a wavy separator carrying the hidden count, and a click opens the run the way IntelliJ does: 4 lines, then 8 more, then the rest, always from the end nearest the caret, so the separator moves away from where you are reading. Hovering the row tints the edge the lines will appear on. A run opening above a visible caret scrolls the view along with it, so the line under the caret stays where it was on screen; a caret off screen is left alone, so a click on a separator never moves what you are looking at. The toolbar's collapse toggle re-collapses partly opened runs along with expanded ones, and a caret caught inside a run moves to the nearest line still on show (#3)
