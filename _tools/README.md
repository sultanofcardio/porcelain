...
- `changelog.py` renders `CHANGELOG.md` from `main` (Keep a Changelog, with the pre-0.7.0 bilingual entries reduced to their English halves) into the timeline. `SHOTS` maps a release entry to the screenshot shown under it. Point `CHANGELOG` at the file; it defaults to a sibling `porcelain` checkout.
