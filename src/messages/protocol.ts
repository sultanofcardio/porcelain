 ...
 * the document's symbol outline. The diff surface renders its own text, so
 * the language servers never see a caret in it; the host runs VS Code's
 * provider commands over the side's document and hands back plain data.
 */
